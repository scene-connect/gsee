...
def test_add_kd_run_gsee():
    df = pd.DataFrame(
        data={
            "global_horizontal": 1000 * np.random.rand(25) / 2,
...
def test_clearness_index_hourly():
    ...
    df = pd.DataFrame(
        data={
            "global_horizontal": [100, 300, 500, 700, 900],
...
def test_convert_to_diurnal():
    df = pd.DataFrame(
        data={"global_horizontal": 1000 * np.random.rand(25) / 2},
        index=pd.date_range(start="2000-05-18", periods=25, freq="D"),
    )
    coords = (45, 10)
...
def test_return_pv():
    ...
    pv = pd.Series(
        ...
        index=pd.date_range(start="2000-01-01", periods=48, freq="D"),
    )
    pv.index.name = "time"
    manager = multiprocessing.Manager()
